		...
		private static void OnTimer(object state)
		{
			try
			{
				lock (_locker)
				{
					if (_list.Count > 0)
					{
						using (FileStream fileStream = File.Open($"{_path}\\payment-{pid:D2}{did:D2}{sid:D2}-{DateTime.Now:yyyyMMdd}.log", FileMode.Append))
						{
					...
					if (RegCount > 0)
					{
						using (FileStream stream = File.Open($"{_path}\\reg-{pid:D2}{did:D2}{sid:D2}-{DateTime.Now:yyyyMMdd}.log", FileMode.Append))
						{
					...
					if (celebBuildDay != day && DateTime.Now.Hour > 2 && DateTime.Now.Hour < 6)
					{
						...
						try
						{
							...
							string str3 = "Complete auto update Celeb in " + str2;
							obj.Info(str3);
						}
						catch (Exception ex2)
						{
							stringBuilder.Append("CelebByList is Error!");
							log.Error(stringBuilder.ToString(), ex2);
						}
					}
				}
			}
