...
namespace Center.Server
{
    public class CenterServer : BaseServer
	{
		...
		private int TimeOpenWorldBoss = int.Parse(ConfigurationManager.AppSettings["TimeOpenWorldBoss"]);

		private int TimeCloseWorldBoss = int.Parse(ConfigurationManager.AppSettings["TimeCloseWorldBoss"]);
		...
		/*public void SendUpdateWorldEvent()
		{
			...
			if ((hour == 0 || hour == 10 || hour == 18) && boss == false)
            {
				boss = true;
            }
			if (!WorldMgr.worldOpen)
			{
				if (hour == TimeOpenWorldBoss)
				{
					this.SendPrivateInfo();
					WorldMgr.SetupWorldBoss(0);
					this.SendSystemNotice(string.Format("Boss thế giới {0} đã mở, tham gia ngay!", WorldMgr.name[0]));
					boss = true;
				}
				else
				{
			...
			if (WorldMgr.worldOpen)
			{
				if (hour == TimeCloseWorldBoss || WorldMgr.current_blood <= 0) // || hour == 20 || WorldMgr.current_blood <= 0)
				{
					if (boss)
					{
						minute1 = System.DateTime.Now.AddMinutes(5.0);
						int currentPVE_ID = WorldMgr.currentPVE_ID;
						this.SendSystemNotice(string.Format("Boss thế giới {0} đã kết thúc. Phòng sẽ đóng sau {1} phút.", WorldMgr.name[currentPVE_ID], 5));
						boss = false;
					}
		...
		public void SendUpdateWorldEvent()
        {
			int minute = TimeZoneInfo.ConvertTime(DateTime.Now, TimeZoneInfo.FindSystemTimeZoneById("SE Asia Standard Time")).Minute;
			int hour = TimeZoneInfo.ConvertTime(DateTime.Now, TimeZoneInfo.FindSystemTimeZoneById("SE Asia Standard Time")).Hour;
			int week = TimeZoneInfo.ConvertTime(DateTime.Now, TimeZoneInfo.FindSystemTimeZoneById("SE Asia Standard Time")).GetWeekOfYear();
			bool CantOpen = DateTime.Now.DayOfWeek == DayOfWeek.Sunday;
			bool CantOpen1 = DateTime.Now.DayOfWeek == DayOfWeek.Saturday;
