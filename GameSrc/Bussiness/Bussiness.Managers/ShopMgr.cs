...
namespace Bussiness.Managers
{
	public static class ShopMgr
	{
		...
		private static Dictionary<int, MinigameShopTemplateInfo> LoadMinigameShopDatabase()
		{
			Dictionary<int, MinigameShopTemplateInfo> list = new Dictionary<int, MinigameShopTemplateInfo>();
			using (ProduceBussiness db = new ProduceBussiness())
			{
				MinigameShopTemplateInfo[] infos = db.GetAllMinigameShop();
				foreach (MinigameShopTemplateInfo info in infos)
				{
		...
		public static bool ReLoad()
		{
			try
			{
				...
				if(tempMinigameShopLists.Count > 0)
                {
					Interlocked.Exchange(ref m_miniShop, tempMinigameShopLists);
                }
				try
				{
		...
		public static MinigameShopTemplateInfo FindMinigameShop(int ID)
		{
			if (m_miniShop.ContainsKey(ID))
			{
				return m_miniShop[ID];
			}
