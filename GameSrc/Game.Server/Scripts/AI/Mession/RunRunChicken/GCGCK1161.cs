...
namespace GameServerScript.AI.Messions
{
	public class GCGCK1161 : AMissionControl
	{
		...
		public override void OnStartGame()
		{
			...
			method_1(int_2);
			method_0(xijcWavNmUo);
		}

		private void method_0(int int_7)
		{
			for (int i = 0; i < int_7; i++)
			{
				Point point = (i < list_2.Count) ? list_2[i] : list_2[base.Game.Random.Next(list_2.Count)];
				list_0.Add(base.Game.CreateNpc(int_5, point.X, point.Y, 0, -1));
			}
		}
		...
		private void method_1(int int_7)
		{
			for (int i = 0; i < int_7; i++)
			{
				Point point = (i < list_3.Count) ? list_3[i] : list_3[base.Game.Random.Next(list_3.Count)];
				list_1.Add(base.Game.CreateNpc(int_5, point.X, point.Y, 0, -1));
			}
		}
		...
		public override void OnNewTurnStarted()
		{
			...
			if (int_3 < xijcWavNmUo && list_0.Count < int_0)
			{
				int num = (xijcWavNmUo - int_3 > int_0 - list_0.Count) ? (int_0 - list_0.Count) : (xijcWavNmUo - int_3);
				if (num > 0)
				{
					method_0(num);
				}
			}
			if (int_4 < int_2 && list_1.Count < int_1)
			{
				int num2 = (int_2 - int_4 > int_1 - list_1.Count) ? (int_1 - list_1.Count) : (int_2 - int_4);
				if (num2 > 0)
				{
					method_1(num2);
				}
		...
		public override bool CanGameOver()
		{
			...
			if (list_1.Count >= int_1 && list_0.Count >= int_0 && base.Game.GetLivedLivings().Count <= 0)
			{
				return true;
			}
