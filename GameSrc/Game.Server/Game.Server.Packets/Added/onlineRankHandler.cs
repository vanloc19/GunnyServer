...
{
    [PacketHandler((short)ePackageType.ONLINE_RANK, "场景用户离开")]
    public class onlineRankHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            List<IGMActive> lists = GmActivityMgr.GetAllGMActionByType(typeof(RankOnline));

            foreach (IGMActive action in lists)
            {
                GSPacketIn pkg = new GSPacketIn((int)ePackageType.ONLINE_RANK);
