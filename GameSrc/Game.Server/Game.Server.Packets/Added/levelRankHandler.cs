...
{
    [PacketHandler((short)ePackageType.LEVEL_RANK, "场景用户离开")]
    public class levelRankHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            List<IGMActive> lists = GmActivityMgr.GetAllGMActionByType(typeof(RankGrade));

            foreach (IGMActive action in lists)
            {
                GSPacketIn pkg = new GSPacketIn((int)ePackageType.LEVEL_RANK);
