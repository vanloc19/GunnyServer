...
{
    [PacketHandler((short)ePackageType.RECHARGE_RANK, "场景用户离开")]
    public class UpdateRechargeRankHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            List<IGMActive> lists = GmActivityMgr.GetAllGMActionByType(typeof(RankRechargeMoney));

            foreach (IGMActive action in lists)
            {
                GSPacketIn pkg = new GSPacketIn((int)ePackageType.RECHARGE_RANK);
