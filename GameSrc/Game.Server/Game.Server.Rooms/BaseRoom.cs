...
namespace Game.Server.Rooms
{
    public class BaseRoom
    {
        ...
        protected int m_playerCount = 0;

        protected int m_placesCount = 10;
        ...
        public bool isWithinLeageTime = false;
        ...
        public void UpdateRoomGameType()
        {
            switch (RoomType)
            {
                ...
                case eRoomType.Match:
                    if (isWithinLeageTime)
                    {
                        GameType = eGameType.Leage;
                    }
                    else
                    {
                        GameType = eGameType.Free;
        ...
        public bool UpdatePosUnsafe2(int pos, bool isOpened, int place, int placeView)
        {
            if (pos < 0 || pos > 9)
                return false;

            this.m_placesState[placeView] = this.m_placesState[pos];
            this.m_playerState[placeView] = (byte)placeView;
            this.m_places[placeView] = this.m_places[pos];
            this.m_placesState[pos] = -1;
            this.m_places[pos] = null;
            this.SendPlaceState();
            return true;
        }
        ...
        public void UpdateGameStyle()
        {
            if (m_host != null && RoomType == eRoomType.Match)
            {
                if (IsAllSameGuild())
                {
                    ...
                    if(isWithinLeageTime)
                    {
                        GameType = eGameType.Guild;//GuildLeague
                    }
                    else
                    {
                ...
                else
                {
                    GameStyle = 0;
                    GameType = eGameType.Free;
                    if (isWithinLeageTime && m_playerCount >= 1)
                    {
                        GameType = eGameType.Leage;
                    }
                    else
        ...
        public bool AddPlayerCampBattle(GamePlayer player)
        {
            ...
            if (index != -1)
            {
                ...
                {
                    m_host = player;
                    UpdatePlayerState(player, 2, false);
                }
                else
                {
                    UpdatePlayerState(player, 0, false);
            ...
            if (index != -1)
            {
                ...
                else
                {
                    ...
                    if (isChangeHost)
                    {
                        ...
                        {
                            gp.Out.SendGameRoomSetupChange(this);
                        }
                    }
                }
                if (RoomType == eRoomType.ConsortiaBattle)
        ...
        public void RemovePlayerAtUnsafe(int pos)
        {
            ...
            if (m_places[pos].KickProtect)
            {
                ...
                GSPacketIn pkg = new GSPacketIn((byte)Packets.ePackageType.SYS_MESSAGE);
                pkg.WriteInt(0);
        ...
        {
            for (int index = 0; index < 8; ++index)//IsQuanChien
            {
                if (m_playerState[index] != 2)
                {
                    m_playerState[index] = 0;
                }
            }
        }
        ...
        {
            string msg = LanguageMgr.GetTranslation("BaseRoom.Msg1");
            string type = LanguageMgr.GetTranslation("BaseRoom.Msg2");
            MapInfo info = MapMgr.FindMapInfo(MapId);
            if (info != null)
            {
                msg = info.Name;
            }
            ...
            {
                type = LanguageMgr.GetTranslation("BaseRoom.Msg3");
            }
        ...
        {
            string msg = LanguageMgr.GetTranslation("BaseRoom.Msg4");
            switch (HardLevel)
            {
                case eHardLevel.Normal:
                    msg = LanguageMgr.GetTranslation("BaseRoom.Msg5");
                    break;
                case eHardLevel.Hard:
                    msg = LanguageMgr.GetTranslation("BaseRoom.Msg6");
                    break;
                case eHardLevel.Terror:
                    msg = LanguageMgr.GetTranslation("BaseRoom.Msg7");
                    break;
            }
        ...
        public int GetDungeonTicketId(int mapId, eHardLevel level)
        {
            int id = 0;
            switch (mapId)
            {
        ...
        public void RemoveAllPlayer()
        {
            ...
            {
                if (m_places[index] != null)
                {
                    RoomMgr.AddAction(new ExitRoomAction(this, m_places[index]));
                    RoomMgr.AddAction(new EnterWaitingRoomAction(m_places[index]));
                }
            }
        }

        public void UpdateRoomEnterType()
        {
            switch (RoomType)
            {
                case eRoomType.Boss:
                case eRoomType.Academy:
                case eRoomType.Dungeon:
                    EnterType = eEnterType.PVE;
                    break;
                case eRoomType.Freedom:
                case eRoomType.Match:
                case eRoomType.EliteGameScore:
                case eRoomType.EliteGameChampion:
                    EnterType = eEnterType.PVP;
                    break;
                default:
                    EnterType = eEnterType.ALL;
                    break;
            }
        }
        ...
        public void UpdateStateViewToPlay(int plance)
        {
            this.m_playerState[plance] = 0;
        }
    }
}