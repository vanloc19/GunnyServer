...
namespace GameServerScript.AI.NPC
{
	public class SixNormalFourBoss : ABrain
	{
		...
		public override void OnStartAttacking()
		{
			...
			else if (keTwPyyubPS == 1)
			{
				method_2();
				keTwPyyubPS++;
			}
			else if (keTwPyyubPS == 2)
			{
				method_0();
				keTwPyyubPS++;
			}
			else if (keTwPyyubPS == 3)
			{
				if (!bool_0)
				{
					method_11();
			...
			else if (keTwPyyubPS == 4)
			{
				method_5();
				keTwPyyubPS++;
			}
			else if (keTwPyyubPS == 5)
			{
				method_9();
				keTwPyyubPS++;
			}
		...
		{
			base.Body.BeatDirect(player_0, "", 4000, int_1, 1);
		}
		...
		private void method_12()
		{
			...
			simpleNpc_0 = (base.Game as PVEGame).CreateNpc(int_2, base.Body.X, base.Body.Y, 1, base.Body.Direction, config);
			bool_0 = true;
