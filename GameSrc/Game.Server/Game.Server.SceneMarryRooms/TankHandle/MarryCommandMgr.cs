	...
	public class MarryCommandMgr
	{
		...
		protected int SearchCommandHandlers(Assembly assembly)
		{
			...
			foreach (Type type in types)
			{
				...
				{
					MarryCommandAttbute[] customAttributes = (MarryCommandAttbute[])type.GetCustomAttributes(typeof(MarryCommandAttbute), inherit: true);
					if (customAttributes.Length != 0)
					{
						num++;
						RegisterCommandHandler(customAttributes[0].Code, Activator.CreateInstance(type) as IMarryCommandHandler);
					}
				}
			}
			return num;
