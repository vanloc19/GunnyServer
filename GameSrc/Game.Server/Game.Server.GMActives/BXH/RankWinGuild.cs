...
{
    public class RankGuildWin : IGMActive
    {
        public RankGuildWin(GmActivityInfo gmActive, List<UserGmActivityInfo> listUsersGmInfo) : base(gmActive, listUsersGmInfo)
        {
            //TO DO
        }
        ...
        {
            GmActivityMgr.UpdateWinGuild += GmActivityMgr_UpdateProperties;
        }
            ...
            if (!m_gmActive.IsAvalible && !IsLockAuto)
            {
                ...
                foreach (GmGiftInfo giftPackage in allGiftPackage)
                {
                    ...
                    if (rankLists.Count >= conditions[0].conditionValue)
                    {
                        ...
                        LanguageMgr.GetTranslation("Phần thưởng Top Win Guild!"), string.Format("Đây là thư gửi từ hệ thống đua top {0} trong hấp dẫn. vui lòng không reply!", m_gmActive.activityName));
                    }
                }
            }
        }
