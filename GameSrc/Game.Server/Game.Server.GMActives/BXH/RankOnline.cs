...
{
    public class RankOnline : IGMActive
    {
        public RankOnline(GmActivityInfo gmActive, List<UserGmActivityInfo> listUsersGmInfo) : base(gmActive, listUsersGmInfo)
        {
            //TO DO
        }
        ...
        {
            GmActivityMgr.UpdateOnline += GmActivityMgr_UpdateOnline;
        }

        private void GmActivityMgr_UpdateOnline(GamePlayer player, int value)
        {
            ...
            if (!m_gmActive.IsAvalible && !IsLockAuto)
            {
                ...
                foreach (GmGiftInfo giftPackage in allGiftPackage)
                {
                    ...
                    if (rankLists.Count >= conditions[0].conditionValue)
                    {
                        ...
                        LanguageMgr.GetTranslation("Phần thưởng Top Online!!"), string.Format("Đây là thư gửi từ hệ thống đua top {0} trong hấp dẫn. vui lòng không reply!", m_gmActive.activityName));
                    }
                }
            }
        }
