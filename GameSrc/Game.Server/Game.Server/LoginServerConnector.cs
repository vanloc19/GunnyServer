	...
	public class LoginServerConnector : BaseConnector
	{
		...
		public void HandleReload(GSPacketIn packet)
		{
			...
			switch (type)
			{
				...
				case eReloadType.newtitle:
					...
					break;
				case eReloadType.subactive:
					val = SubActiveMgr.ReLoad();
					break;
			}
		...
		public void HandleSysMess(GSPacketIn packet)
		{
			if (packet.ReadInt() == 1)
			{
				int playerId = packet.ReadInt();
				string str = packet.ReadString();
				WorldMgr.GetPlayerById(playerId)?.Out.SendMessage(eMessageType.ChatERROR, LanguageMgr.GetTranslation("LoginServerConnector.HandleSysMess.Msg1", str));
			}
		}
		...
		{
			WorldMgr.UpdateIsLeagueOpen(pkg.ReadBoolean());
		}
