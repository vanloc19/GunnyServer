...
namespace Fighting.Server.GameObjects
{
	...
	// Giới hạn CS của bot không quá 100k
	if (character.IsAutoBot)
	{
		// Tính tổng chỉ số (Attack + Defence + Agility)
		double totalCS = proxyPlayer.BaseAttack + proxyPlayer.BaseDefence + proxyPlayer.BaseAgility;
		if (totalCS > 100000)
		{
			// Scale down tất cả chỉ số về 100k
			double scaleFactor = 100000.0 / totalCS;
			m_baseAttack = proxyPlayer.BaseAttack * scaleFactor;
			m_baseDefence = proxyPlayer.BaseDefence * scaleFactor;
			m_baseAglilty = proxyPlayer.BaseAgility * scaleFactor;
			m_baseBlood = proxyPlayer.BaseBlood;  // Giữ nguyên HP
		}
		else
		{
			// Bot yếu hơn 100k thì giữ nguyên
	...
	else
	{
		...
		m_baseBlood = proxyPlayer.BaseBlood;
			...
			m_tcpEndPoint = proxyPlayer.TcpEndPoint;
		}
		...
		{
			if (value > 0) m_client.SendAddEliteGameScore(PlayerCharacter.ID, value);
			return value;
		}
		public int RemoveEliteScore(int value)
		{
			if (value > 0) m_client.SendRemoveEliteGameScore(PlayerCharacter.ID, value);
			return value;
		}
		public void SendWinEliteChampion(int blood, int win)
		{
			m_client.SendEliteGameWinUpdate(PlayerCharacter.ID, blood, win);
		}
		...
		public int RemoveMoney(int value, bool isConsume)
		{
		...
		{
			m_client.SendPlayerLeageScoure(m_character.ID, isWin, value);
			return 0;
		}
        ...
        public void PVPFightNotice(string msg)
        {
			if (msg != null)
			{
                Game.Server.Managers.WorldMgr.SendSysTipNotice(msg);
			}
		}
