...
namespace Game.Server.LittleGame.Objects
{
    public class Bogu
    {
        ...
        private void Escaping(object state)
        {
            ...
            foreach (var player in Catchers)
            {
        ...
        public Bogu(int id, int x, int y, string model, int size)
        {
            ...
            {
                case 0:
                    m_moverTimer = new Timer(RandomMove, null, 3000 + ThreadSafeRandom.NextStatic(100, 500),
                        4555 + ThreadSafeRandom.NextStatic(1000, 1500));
                    Score = LittleGameWorldMgr.Config.SmallBoguScore;
                    HP = LittleGameWorldMgr.Config.SmallBoguHP;
                    break;
                case 1:
                    m_moverTimer = new Timer(RandomMove, null, 3000 + ThreadSafeRandom.NextStatic(100, 500),
                        4555 + ThreadSafeRandom.NextStatic(2500, 3500));
                    Score = LittleGameWorldMgr.Config.MediumBoguScore;
                    HP = LittleGameWorldMgr.Config.MediumBoguHP;
                    break;
                case 2:
                    m_moverTimer = new Timer(RandomMove, null, 3000 + ThreadSafeRandom.NextStatic(100, 500),
                        4555 + ThreadSafeRandom.NextStatic(4500, 5500));
                ...
                case 3:
                    m_moverTimer = new Timer(RandomMove, null, 3000 + ThreadSafeRandom.NextStatic(100, 500),
                        4555 + ThreadSafeRandom.NextStatic(6500, 8500));
        ...
        {
            if (LittleGameWorldMgr.PlayerCount == 0)
            {
                return;
            }
            ...
            var p = LittleGameWorldMgr.Map.points[ThreadSafeRandom.NextStatic(LittleGameWorldMgr.Map.points.Count - 1)];
            X = p.X;
            Y = p.Y;
