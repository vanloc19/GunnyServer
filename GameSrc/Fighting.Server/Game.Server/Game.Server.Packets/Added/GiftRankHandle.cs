...
{
    [PacketHandler((short)ePackageType.GIFT_RANK, "场景用户离开")]
    public class GiftRankHandle : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            List<IGMActive> lists = GmActivityMgr.GetAllGMActionByType(typeof(GiftRank));

            foreach (IGMActive action in lists)
            {
                GSPacketIn pkg = new GSPacketIn((int)ePackageType.GIFT_RANK);
