...
{
    [PacketHandler((short)ePackageType.WIN_RANK, "场景用户离开")]
    public class winRankHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            List<IGMActive> lists = GmActivityMgr.GetAllGMActionByType(typeof(RankWin));

            foreach (IGMActive action in lists)
            {
                GSPacketIn pkg = new GSPacketIn((int)ePackageType.WIN_RANK);
