...
{
    [PacketHandler((short)ePackageType.WIN_GUILD_RANK, "场景用户离开")]
    public class winGuildRankHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            List<IGMActive> lists = GmActivityMgr.GetAllGMActionByType(typeof(RankGuildWin));

            foreach (IGMActive action in lists)
            {
                GSPacketIn pkg = new GSPacketIn((int)ePackageType.WIN_GUILD_RANK);
