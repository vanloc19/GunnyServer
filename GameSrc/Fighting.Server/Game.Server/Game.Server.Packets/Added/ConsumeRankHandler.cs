...
{
    [PacketHandler((short)ePackageType.CONSUME_RANK, "场景用户离开")]
    public class ConsumeRankHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            List<IGMActive> lists = GmActivityMgr.GetAllGMActionByType(typeof(RankConsumeMoney));
            
            foreach (IGMActive action in lists)
            {
                GSPacketIn pkg = new GSPacketIn((int)ePackageType.CONSUME_RANK);
