...
namespace Game.Server.Packets.Client
{
    ...
    public class FigSpiritUpGradeHandler : IPacketHandler
    {
        ...
        private static int
            golddenLv = GameProperties.FightSpiritMaxLevel; //FightSpiritTemplateMgr.GOLDEN_LEVEL();        
        ...
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            //Console.WriteLine("autoBuyId:{0},  goodsId:{1}, type:{2}, templeteId:{3}, fightSpiritId:{4}, equipPlace:{5}, place:{6}, count:{7}",
            //    autoBuyId, goodsId, type, templeteId, fightSpiritId, equipPlace, place, count);
            ...
            int num = 1; //exp
            int dir = 0;
            //Console.WriteLine("FigSpiritIdValue {0}, isMaxLevel {1}, golddenLv {2}", gemStone.FigSpiritIdValue, isMaxLevel, golddenLv);
            ...
            if (!isMaxLevel)
            {
                ...
                if (curGem == null)
                {
                    ...
                    client.Out.SendMessage(eMessageType.Normal,
                        LanguageMgr.GetTranslation("FigSpiritUpGradeHandler.WrongPlace"));
                    return 0;
                }
                ...
                switch (autoBuyId)
                {
                    case 1:
                        ...
                        //int currCount = itemCount - needCount;                       
                        //Console.WriteLine("FigSpiritUpGradeHandler needCount:{0}, itemCount: {1}, currCount: {2}, curGem.exp: {3}, Level: {4}, NextLv: {5}", needCount, itemCount, currCount, curGem.exp, curGem.level, exps[curGem.level + 1]);
                        break;
                    default:
                ...
                if (curGem.level < golddenLv)
                {
                    ...
                    if (isUp)
                    {
                        curGem.level++;
                        curGem.exp = 0;
                        GmActivityMgr.OnFigSpiritUp(client.Player, 1);
                    }
                }
        ...
        {
            return SpiritIdValue[0].Split(',')[0] == golddenLv.ToString() &&
                   SpiritIdValue[1].Split(',')[0] == golddenLv.ToString() &&
                   SpiritIdValue[2].Split(',')[0] == golddenLv.ToString();
        }
