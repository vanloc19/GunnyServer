...
{
    [PacketHandler((short)ePackageType.WIN_LEAGUE_RANK, "场景用户离开")]
    public class winLeagueRankHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            List<IGMActive> lists = GmActivityMgr.GetAllGMActionByType(typeof(RankLeagueWin));

            foreach (IGMActive action in lists)
            {
                GSPacketIn pkg = new GSPacketIn((int)ePackageType.WIN_LEAGUE_RANK);
