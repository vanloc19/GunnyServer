...
{
    [PacketHandler((short)ePackageType.BUGLE_RANK, "场景用户离开")]
    public class BugbleRankHandle : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            List<IGMActive> lists = GmActivityMgr.GetAllGMActionByType(typeof(RankBigBugle));

            foreach (IGMActive action in lists)
            {
                GSPacketIn pkg = new GSPacketIn((int)ePackageType.BUGLE_RANK);
