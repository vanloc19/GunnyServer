...
{
	[PacketHandler((int)ePackageType.LABYRINTH, "场景用户离开")]
	public class LabyrinthHandler : IPacketHandler
	{
		public int HandlePacket(GameClient client, GSPacketIn packet)
		{
			...
			case LabyrinthPackageType.SPEEDED_UP_CLEAN_OUT:
			{
				...
				if (client.Player.MoneyDirect(num2, IsAntiMult: false))
				{
			...
			{
				int num4 = (packet.ReadBoolean() ? 1 : 0);
				packet.ReadBoolean();
				if (num4 != 0)
				{
					int num3 = client.Player.LabyrinthTryAgainMoney();
					if (client.Player.RemoveMoney(num3, isConsume: false) > 0)
					{
						laby.completeChallenge = true;
						laby.isInGame = true;
