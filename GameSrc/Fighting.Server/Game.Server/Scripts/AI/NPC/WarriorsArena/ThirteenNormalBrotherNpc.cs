...
namespace GameServerScript.AI.NPC
{
	public class ThirteenNormalBrotherNpc : ABrain
	{
		...
		private SimpleBoss nkDsoVfybVd;

		protected Player m_targer;

		private PhysicalObj physicalObj_0;

		private int ootsomGdKu0;

		private int int_1;

		private int int_2;

		public ThirteenNormalBrotherNpc()
		{
			
			this.ootsomGdKu0 = 13105;
			this.int_1 = 200;
			this.int_2 = 50;
		...
		private void method_0()
		{
			...
			if (players.Count <= 0)
			{
				...
				this.nkDsoVfybVd.Properties1 = 1;
			...
			else
			{
				...
				foreach (Player player in base.Game.GetAllLivingPlayers())
				{
					...
					player.AddEffect(new AddDamageTurnEffect(this.int_1, 2), 7000);
					player.AddEffect(new AddGuardTurnEffect(this.int_2, 2), 7000);
				}
				(base.Game as PVEGame).SendObjectFocus(this.nkDsoVfybVd, 0, 9000, 0);
				this.nkDsoVfybVd.Say("Có chuyện gì vậy? Cúng tế bị sai cmnr.", 0, 10500, 2000);
			...
			this.nkDsoVfybVd.Delay = base.Game.GetLowDelayTurn() - 1;
		...
		public override void OnBeginSelfTurn()
		{
			...
			if (this.nkDsoVfybVd == null)
			{
				SimpleBoss[] simpleBossArray = ((PVEGame)base.Game).FindLivingTurnBossWithID(this.ootsomGdKu0);
				if (simpleBossArray.Length != 0)
				{
					this.nkDsoVfybVd = simpleBossArray[0];
				}
			}
		}
