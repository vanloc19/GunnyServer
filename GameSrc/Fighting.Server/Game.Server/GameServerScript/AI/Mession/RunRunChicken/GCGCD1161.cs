...
namespace GameServerScript.AI.Messions
{
	public class GCGCD1161 : AMissionControl
	{
		...
		private void method_1(int int_8)
		{
			for (int i = 0; i < int_8; i++)
			{
				Point point = list_3[i];
				list_1.Add(base.Game.CreateNpc(int_6, point.X, point.Y, 0, -1));
			}
		}
		...
		public override void OnNewTurnStarted()
		{
			...
			if (int_4 < int_2 && list_0.Count < int_0)
			{
				int num = (int_2 - int_4 > int_0 - list_0.Count) ? (int_0 - list_0.Count) : (int_2 - int_4);
				if (num > 0)
				{
					method_0(num);
				}
			}
			if (int_5 < int_3 && list_1.Count < int_1)
			{
				int num2 = (int_3 - int_5 > int_1 - list_1.Count) ? (int_1 - list_1.Count) : (int_3 - int_5);
				if (num2 > 0)
				{
					method_1(num2);
				}
