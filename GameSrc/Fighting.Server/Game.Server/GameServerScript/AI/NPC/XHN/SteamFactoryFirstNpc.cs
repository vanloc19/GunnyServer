...
{
    public class SteamFactoryFirstNpc : ABrain
    {
            ...
            if (dis > 97)
            {
                ...
                if (player.Y < 420 && player.X < 210)
                {
                    if (Body.Y > 420)
                    {
                        if (Body.X - dis < 50)
                        {
                            Body.NewFlyTo(25, Body.Y, "walk", 1200, "", ((SimpleNpc)Body).NpcInfo.speed, new LivingCallBack(Jump));
                        }
                        else
                        {
                            Body.NewFlyTo(Body.X - dis, Body.Y, "walk", 1200, "", ((SimpleNpc)Body).NpcInfo.speed, new LivingCallBack(MoveBeat));
                        }
                    }
                    else
                    {
                ...
                else
                {
                    if (Body.Y < 420)
                    {
                        ...
                        //else
                        //{
                        //    Body.FlyTo(Body.X + dis, Body.Y, "walk", 1200, new LivingCallBack(MoveBeat));
                        //}
                        if (Body.X + dis > 200)
                        {
                            Body.NewFlyTo(200, Body.Y, "walk", 1200, "", ((SimpleNpc)Body).NpcInfo.speed, new LivingCallBack(Fall));
                        }
                    }
                    else
                    {
