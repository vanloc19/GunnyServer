...
{
	public class ThirteenTerrorDevilBoss : ABrain
	{
		private int int_0;
		...
		private List<PhysicalObj> OaksAzssyln;

		private int yResmeCaKsr;

		private int int_1;

		public ThirteenTerrorDevilBoss()
		{
			
			this.list_0 = new List<PhysicalObj>();
			this.OaksAzssyln = new List<PhysicalObj>();
			this.yResmeCaKsr = 13307;
			this.int_1 = 13310;
			
		}

		private void LbQsAqcXidh()
		{
			this.m_targer = base.Game.FindRandomPlayer();
			base.Body.Say("Coi ta cắt chym tụi mi đây.", 0, 1000);
			base.Body.MoveTo(this.m_targer.X, this.m_targer.Y, "fly", 1500, new LivingCallBack(this.method_10), 10);
		}
		...
		private void method_10()
		{
			...
			base.Body.CallFuction(new LivingCallBack(this.method_11), 2000);
		}
		...
		private void method_11()
		{
			...
			base.Body.MoveTo(num, num1, "fly", 1000, new LivingCallBack(this.method_13), 10);
		}
		...
		private void method_13()
		{
			if (this.physicalObj_0 != null)
			{
				...
				else
				{
					...
					base.Body.CallFuction(new LivingCallBack(this.method_12), 5700);
				}
				base.Body.CallFuction(new LivingCallBack(this.vEpsAnQnqFw), 5000);
				return;
			}
			if (this.OaksAzssyln.Count > 0)
			{
				...
				if (players.Count >= 4)
				{
					...
					base.Body.CallFuction(new LivingCallBack(this.method_15), 2500);
					base.Body.CallFuction(new LivingCallBack(this.method_14), 3500);
					foreach (PhysicalObj oaksAzssyln in this.OaksAzssyln)
					{
						oaksAzssyln.PlayMovie("beatA", 5000, 0);
					}
					this.simpleBoss_0.PlayMovie("ready", 6500, 0);
					this.simpleBoss_0.PlayMovie("standC", 7200, 2000);
					base.Body.CallFuction(new LivingCallBack(this.method_12), 9200);
					return;
				}
				base.Body.CurrentDamagePlus = 5f;
				foreach (PhysicalObj physicalObj in this.OaksAzssyln)
				{
					...
					base.Body.CallFuction(new LivingCallBack(this.method_15), 4600);
				}
			}
		}
		...
		{
			this.OaksAzssyln.Add((base.Game as PVEGame).Createlayer(this.simpleBoss_0.X - 100, this.simpleBoss_0.Y - 170, "", "asset.game.ten.laotie2", "", 1, 0));
			this.OaksAzssyln.Add((base.Game as PVEGame).Createlayer(this.simpleBoss_0.X - 80, this.simpleBoss_0.Y - 200, "", "asset.game.ten.laotie2", "", 1, 0));
			this.OaksAzssyln.Add((base.Game as PVEGame).Createlayer(this.simpleBoss_0.X + 100, this.simpleBoss_0.Y - 170, "", "asset.game.ten.laotie", "", 1, 0));
			this.OaksAzssyln.Add((base.Game as PVEGame).Createlayer(this.simpleBoss_0.X + 80, this.simpleBoss_0.Y - 200, "", "asset.game.ten.laotie", "", 1, 0));
		}
		...
		private void method_15()
		{
			foreach (PhysicalObj oaksAzssyln in this.OaksAzssyln)
			{
				base.Game.RemovePhysicalObj(oaksAzssyln, true);
			}
			this.OaksAzssyln = new List<PhysicalObj>();
		}
		...
		private void method_16()
		{
			...
			base.Body.CallFuction(new LivingCallBack(this.method_17), 5000);
		}
		...
		private void method_4()
		{
			foreach (Player allLivingPlayer in base.Game.GetAllLivingPlayers())
			{
				this.OaksAzssyln.Add(((PVEGame)base.Game).Createlayer(allLivingPlayer.X, allLivingPlayer.Y - 50, "", "asset.game.ten.laotie", "born", 1, 1));
			}
		}
		...
		private void method_7()
		{
			...
			this.simpleNpc_0 = ((SimpleBoss)base.Body).CreateChild(this.int_1, num, 1000, false, livingConfig);
		...
		public override void OnBeginSelfTurn()
		{
			...
			{
				SimpleBoss[] simpleBossArray = ((PVEGame)base.Game).FindLivingTurnBossWithID(this.yResmeCaKsr);
