...
namespace Game.Server.Rooms
{
    public partial class RoomMgr
    {
        ...
        public static BaseChristmasRoom ChristmasRoom
        {
            get
            {
                return RoomMgr.baseChristmasRoom_0;
            }
        }
        ...
        public static bool Setup(int maxRoom)
        {
            ...
            m_worldbossRoom = new BaseWorldBossRoom();
        ...
        public static List<BaseRoom> GetAllPveRooms()
        {
            ...
            lock (m_rooms)
            {
                foreach (BaseRoom room in m_rooms)
                {
                    if (room.IsUsing)
                    {
                        if (room.RoomType == eRoomType.Dungeon || room.RoomType == eRoomType.Academy || room.RoomType == eRoomType.Boss)
                        {
                            list.Add(room);
                        }
                    }
        ...
        public static List<BaseRoom> GetAllMatchRooms()
        {
            ...
            lock (m_rooms)
            {
                foreach (BaseRoom room in m_rooms)
                {
                    if (room.IsUsing)
                    {
                        if (room.RoomType == eRoomType.Match || room.RoomType == eRoomType.Freedom || room.RoomType == eRoomType.EliteGameScore || room.RoomType == eRoomType.EliteGameChampion)
                        {
                            list.Add(room);
                        }
                    }
        ...
        public static List<BaseRoom> GetUsingRoom(int type)
        {
            ...
            lock (m_rooms)
            {
                foreach (BaseRoom baseRoom in m_rooms)
                {
        ...
        {
            foreach (BaseRoom room in GetUsingRoom(type))
            {
        ...
        public static void ChangePlaceToView(BaseRoom room, int pos, bool isOpened, int place, int placeView)
        {
            RoomMgr.AddAction((IAction)new Game.Server.Rooms.ChangePlaceToView(room, pos, isOpened, place, placeView));
        }
