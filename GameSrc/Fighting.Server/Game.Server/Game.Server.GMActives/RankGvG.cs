...
{
    public class RankGvG : IGMActive
    {
        public RankGvG(GmActivityInfo gmActive, List<UserGmActivityInfo> listUsersGmInfo) : base(gmActive, listUsersGmInfo)
        {
            //TO DO
        }
        ...
        {
            GmActivityMgr.UpdateGvG += GmActivityMgr_UpdateProperties;
        }
        ...
        public override void DoAction()
        {
            if (!m_gmActive.IsAvalible && !IsLockAuto)
            {
                ...
                int rank = 1;
                listTop.AppendLine($"List TOP 10 Copa da Quantidade GvG ");
                foreach (UserGmActivityInfo r in rankLists)
                {
                    listTop.AppendLine($"Rank {rank}: {r.NickName} Quantidade GvG:{r.Value}");
                    rank++;
                }
