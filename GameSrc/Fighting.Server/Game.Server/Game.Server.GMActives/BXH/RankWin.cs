...
{
    public class RankWin : IGMActive
    {
        public RankWin(GmActivityInfo gmActive, List<UserGmActivityInfo> listUsersGmInfo) : base(gmActive, listUsersGmInfo)
        {
            //TO DO
        }
        ...
        {
            GmActivityMgr.UpdateWin += GmActivityMgr_UpdateProperties;
        }
            ...
            if (!m_gmActive.IsAvalible && !IsLockAuto)
            {
                ...
                foreach (GmGiftInfo giftPackage in allGiftPackage)
                {
                    ...
                    if (rankLists.Count >= conditions[0].conditionValue)
                    {
                        ...
                        LanguageMgr.GetTranslation("Phần thưởng Top Win!"), string.Format("Đây là thư gửi từ hệ thống đua top {0} trong hấp dẫn. vui lòng không reply!", m_gmActive.activityName));
                    }
                }
            }
        }
