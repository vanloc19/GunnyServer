...
{
    public class RankBigBugle : IGMActive
    {
        public RankBigBugle(GmActivityInfo gmActive, List<UserGmActivityInfo> listUsersGmInfo) : base(gmActive, listUsersGmInfo)
        {
            //TO DO
        }
        ...
        {
            GmActivityMgr.UpdateUseBigBugle += GmActivityMgr_UpdateUseBigBugle;
        }

        private void GmActivityMgr_UpdateUseBigBugle(GamePlayer player, int value)
        {
        ...
        public override void DoAction()
        {
            if (!m_gmActive.IsAvalible && !IsLockAuto)
            {
                ...
                int rank = 1;
                listTop.AppendLine($"List TOP 10 Copa da Online ");
                foreach (UserGmActivityInfo r in rankLists)
                {
                    listTop.AppendLine($"Rank {rank}: {r.NickName} Online:{r.Value}");
                    rank++;
                }
                ...
                foreach (GmGiftInfo giftPackage in allGiftPackage)
                {
                    ...
                    if (rankLists.Count >= conditions[0].conditionValue)
                    {
                        ...
                        LanguageMgr.GetTranslation("Top Lực Chiến (Tân Vương)"), string.Format("Đây là thư gửi từ hệ thống đua top {0} trong hấp dẫn. vui lòng không reply!", m_gmActive.activityName));
                    }
                }
            }
        }
