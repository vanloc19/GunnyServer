					...
					switch (line.Split(' ')[0])
					{
						...
						case "title&reload":
							if (NewTitleMgr.ReLoad())
								Console.WriteLine("Reload NewTitle Success!");
							else
								Console.WriteLine("Reload NewTitle Error - Please Check Again!");
							continue;
						...
						case "shop&reload":
							...
							continue;
						#region Reload By Hungg
						case "reloadActive":
							if(ActiveMgr.ReLoad())
                            {
								Console.WriteLine("Reload ActiveMgr Suceess.");
							}
							else
                            {
								Console.WriteLine("Reload ActiveMgr Failed.");
							}
							continue;
						case "hapdanA":
							if (GmActivityMgr.ReLoad(true))
                            {
								Console.WriteLine("Reload With Rescan = True");
                            }
							else
                            {
								Console.WriteLine("Error");
							...
							continue;
						case "hapdanB":
							if (GmActivityMgr.ReLoad(false))
							{
								Console.WriteLine("Reload With Rescan = False");
							}
							else
							{
								Console.WriteLine("Error");
						...
						case "dotbien":
                            {
								if (SubActiveMgr.ReLoad())
									Console.WriteLine("Reload SubActive Success.");
								else
									Console.WriteLine("Reload SubActive Error.");
                            }
							continue;
						...
						case "itembox&reload":
							...
							continue;
						case "ctask&reload":
							if (ConsortiaTaskMgr.Init())
							{
								Console.WriteLine("ConsortiaTaskMgr is Reload!");
							}
							else
							{
								Console.WriteLine("ConsortiaTaskMgr is Error!");
							}
							continue;
						case "pet&reload":
							if (PetMgr.Init())
						...
						case "test":
                            {
								...
								{
									Console.ForegroundColor = ConsoleColor.Green;
									Console.Write("Little Game Has Actived");
									Console.ResetColor();
								}
								else
								{
									Console.ForegroundColor = ConsoleColor.Green;
									Console.Write("Little Game Has Closed");
									Console.ResetColor();
								}
								continue;
                            }
						case "task":
                            {
								TaskMgr.Init();
								Console.ForegroundColor = ConsoleColor.Green;
								Console.Write("Little Game Has Reloaded");
								Console.ResetColor();
								continue;
                            }
                        case "reloadall":
