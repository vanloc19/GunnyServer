...
namespace Bussiness.Managers
{
	public class ItemBoxMgr
	{
		...
		public static bool CreateItemBox(int DateId, List<ItemBoxInfo> tempBox, List<ItemInfo> itemInfos, SpecialItemDataInfo specialInfo)
		{
			...
			Console.WriteLine($"After_{maxRound}");
			...
			foreach (ItemBoxInfo itemBoxInfo in itemBoxInfoList2)
			{
				...
				switch (itemBoxInfo.TemplateId)
				{
					...
					case 11107:
						...
						continue;
					case -2100://thuy tinh
						specialInfo.CrystalScore += itemBoxInfo.ItemCount;
						continue;
				}
		...
		public static bool CreateLotteryItemBox(int DateId, List<ItemBoxInfo> tempBox, List<ItemInfo> itemInfos, SpecialItemDataInfo specialInfo, List<ItemBoxInfo> resultBox)
		{
			new List<ItemBoxInfo>();
			List<ItemBoxInfo> source = FindItemBox(DateId);
			if (tempBox != null && tempBox.Count > 0)
			...
			foreach (ItemBoxInfo itemBoxInfo in itemBoxInfoList2)
			{
				...
				if (fromTemplate != null)
				{
					...
					if (resultBox == null)
					{
						resultBox = new List<ItemBoxInfo>();
					}
					resultBox.Add(itemBoxInfo);
				}
			}
			return true;
		}
