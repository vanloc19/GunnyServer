...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1099 : BasePetEffect
    {
        ...
        {
            AE1099 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1099) as AE1099;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                {
                    ally.AddPetEffect(
                        new CE1099(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
            }
        }
    }
