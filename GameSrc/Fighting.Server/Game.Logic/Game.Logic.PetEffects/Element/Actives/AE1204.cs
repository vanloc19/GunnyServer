...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1204 : BasePetEffect
    {
        ...
        {
            AE1204 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1204) as AE1204;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                foreach (Player enemy in Enemies)
                {
                    ...
                    enemy.AddPetEffect(
                        new CE1204(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
