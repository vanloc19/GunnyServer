...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1092 : BasePetEffect
    {
        ...
        {
            AE1092 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1092) as AE1092;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                foreach (Player ally in allies)
                {
                    ...
                    ally.AddPetEffect(
                        new CE1092(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
            }
        }
    }
