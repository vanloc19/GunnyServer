...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1202 : BasePetEffect
    {
        ...
        {
            AE1202 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1202) as AE1202;
            if (effect != null)
            {
        ...
        private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
        {
            if (IsTrigger)
            {
                IsTrigger = false;
                target.AddPetEffect(
                    new CE1202(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
