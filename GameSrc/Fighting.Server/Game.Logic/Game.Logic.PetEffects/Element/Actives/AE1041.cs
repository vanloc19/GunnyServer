...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1041 : BasePetEffect
    {
        ...
        {
            AE1041 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1041) as AE1041;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1041(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}