...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1208 : BasePetEffect
    {
        ...
        {
            AE1208 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1208) as AE1208;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                foreach (Player enemy in Enemies)
                {
                    ...
                    enemy.RemovePetMP(10);
                    enemy.AddPetEffect(
                        new CE1208(0, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
