...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1209 : BasePetEffect
    {
        ...
        {
            AE1209 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1209) as AE1209;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                foreach (Player enemy in Enemies)
                {
                    ...
                    enemy.RemovePetMP(30);
                    enemy.AddPetEffect(
                        new CE1209(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
