...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1199 : BasePetEffect
    {
        ...
        {
            AE1199 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1199) as AE1199;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1199(1, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
