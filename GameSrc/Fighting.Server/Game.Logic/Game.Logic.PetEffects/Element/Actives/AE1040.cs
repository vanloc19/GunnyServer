...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1040 : BasePetEffect
    {
        ...
        {
            AE1040 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1040) as AE1040;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1040(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}