...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1180 : BasePetEffect
    {
        ...
        {
            AE1180 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1180) as AE1180;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1180(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}