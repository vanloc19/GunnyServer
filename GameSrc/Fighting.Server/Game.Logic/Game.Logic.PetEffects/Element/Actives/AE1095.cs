...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1095 : BasePetEffect
    {
        ...
        {
            AE1095 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1095) as AE1095;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                {
                    ally.AddPetEffect(
                        new CE1095(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
            }
        }
    }
