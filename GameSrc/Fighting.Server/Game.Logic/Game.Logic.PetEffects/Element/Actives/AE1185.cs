...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1185 : BasePetEffect
    {
        ...
        {
            AE1185 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1185) as AE1185;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1185(0, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}