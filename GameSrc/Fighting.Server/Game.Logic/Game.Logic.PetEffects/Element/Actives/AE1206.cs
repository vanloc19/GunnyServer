...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1206 : BasePetEffect
    {
        ...
        {
            AE1206 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1206) as AE1206;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                foreach (Player enemy in Enemies)
                {
                    ...
                    enemy.AddPetEffect(
                        new CE1206(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
