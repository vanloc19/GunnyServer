...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1171 : BasePetEffect
    {
        ...
        {
            AE1171 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1171) as AE1171;
            if (effect != null)
            {
        ...
        private void Player_PlayerBuffSkillPet(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                foreach (Player enemy in enemies)
                {
                    ...
                    enemy.AddPetEffect(
                        new CE1171(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString(), player),
                        0);
                }
            }
        }
