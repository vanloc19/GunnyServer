...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1042 : BasePetEffect
    {
        ...
        {
            AE1042 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1042) as AE1042;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1042(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}