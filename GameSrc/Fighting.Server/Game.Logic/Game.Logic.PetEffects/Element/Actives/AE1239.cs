...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1239 : BasePetEffect
    {
        ...
        {
            AE1239 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1239) as AE1239;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1239(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}