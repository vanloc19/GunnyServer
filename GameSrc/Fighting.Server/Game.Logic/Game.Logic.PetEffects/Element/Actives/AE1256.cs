...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1256 : BasePetEffect
    {
        ...
        {
            AE1256 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1256) as AE1256;
            if (effect != null)
            {
        ...
        private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
        {
            if (IsTrigger)
            {
                IsTrigger = false;
                target.AddPetEffect(
                    new CE1256(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
