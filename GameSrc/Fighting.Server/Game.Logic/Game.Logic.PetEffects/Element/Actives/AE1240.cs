...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1240 : BasePetEffect
    {
        ...
        {
            AE1240 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1240) as AE1240;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1240(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}