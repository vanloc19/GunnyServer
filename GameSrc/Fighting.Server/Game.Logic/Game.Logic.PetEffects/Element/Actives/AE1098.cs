...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1098 : BasePetEffect
    {
        ...
        {
            AE1098 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1098) as AE1098;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                {
                    ally.AddPetEffect(
                        new CE1098(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
            }
        }
    }
