...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1049 : BasePetEffect
    {
        ...
        {
            AE1049 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1049) as AE1049;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1049(0, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}