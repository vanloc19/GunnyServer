...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1097 : BasePetEffect
    {
        ...
        {
            AE1097 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1097) as AE1097;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                {
                    ally.AddPetEffect(
                        new CE1097(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
            }
        }
    }
