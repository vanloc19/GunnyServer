...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1254 : BasePetEffect
    {
        ...
        {
            AE1254 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1254) as AE1254;
            if (effect != null)
            {
        ...
        private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
        {
            if (IsTrigger)
            {
                IsTrigger = false;
                target.AddPetEffect(
                    new CE1254(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
