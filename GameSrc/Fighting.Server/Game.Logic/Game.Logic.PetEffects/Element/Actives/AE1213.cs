...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1213 : BasePetEffect
    {
        ...
        {
            AE1213 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1213) as AE1213;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1213(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
