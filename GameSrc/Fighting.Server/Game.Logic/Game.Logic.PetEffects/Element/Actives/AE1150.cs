...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1150 : BasePetEffect
    {
        ...
        {
            AE1150 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1150) as AE1150;
            if (effect != null)
            {
        ...
        private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
        {
            if (IsTrigger)
            {
                ...
                target.AddPetEffect(
                    new CE1150(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString(), living), 0);
            }
        }
