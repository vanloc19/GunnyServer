...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1230 : BasePetEffect
    {
        ...
        {
            AE1230 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1230) as AE1230;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1230(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
