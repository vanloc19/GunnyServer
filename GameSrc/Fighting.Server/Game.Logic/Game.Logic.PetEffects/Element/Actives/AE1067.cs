...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1067 : BasePetEffect
    {
        ...
        {
            AE1067 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1067) as AE1067;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                player.AddPetEffect(new CE1067(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}