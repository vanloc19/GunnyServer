...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1259 : BasePetEffect
    {
        ...
        {
            AE1259 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1259) as AE1259;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                {
                    ally.AddPetEffect(
                        new CE1259(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
