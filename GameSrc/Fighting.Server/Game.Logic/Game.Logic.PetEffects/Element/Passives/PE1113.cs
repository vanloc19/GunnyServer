...
namespace Game.Logic.PetEffects.Element.Passives
{
    public class PE1113 : BasePetEffect
    {
        ...
        {
            PE1113 effect = living.PetEffectList.GetOfType(ePetEffectType.PE1113) as PE1113;
            if (effect != null)
            {
        ...
        private void Player_AfterKilledByLiving(Living living, Living target, int damageAmount, int criticalAmount)
        {
            if (rand.Next(100) <= 20)
            {
                ...
                if (m_added > 0)
                {
                    ...
                    {
                        target.AddPetEffect(
                            new CE1113(0, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                    }
