...
namespace Game.Logic
{
	public class DropInventory
	{
		...
		public static List<ItemInfo> CopySystemDrop(int copyId, int OpenCount)
		{
			...
			int num5 = GetDropCondiction(eDropType.Copy, copyId.ToString(), "3");
			if (num5 > 0)
			{
				for (int num = 0; num < num3; num++)
				{
					if (GetDropItems(eDropType.Copy, num5, ref itemInfos))
					{
						list.Add(itemInfos[0]);
						itemInfos = null;
					}
			...
			int num6 = GetDropCondiction(eDropType.Copy, copyId.ToString(), "4");
			if (num6 > 0)
			{
		...
		private static bool GetDropItems(eDropType type, int dropId, ref List<ItemInfo> itemInfos)
		{
			...
			try
            {
				...
				int maxRound = ThreadSafeRandom.NextStatic(unFiltItems.Select(s => s.Random).Max());
