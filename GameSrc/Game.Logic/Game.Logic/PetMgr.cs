	...
	public class PetMgr
	{
		...
		public static bool ReLoad()
		{
			try
			{
				...
				if (smethod_0(sbgjaFH96yj7P79ekB, EvHSgt7QX8VsEd0yEZ, R11rtWo1p4Y2vEtvxF, m8Jor90v2RFloePXnt, YHCrVLAS0QefBk0Ur5, w2gAkTbu06gsoxFCZU, PnKXOFTHDG3PZUPkBD, w84AGjgion3EqNro8x, tempExpItemPrices))
				{
					try
					{
		...
		private static bool smethod_0(Dictionary<string, PetConfig> sbgjaFH96yj7P79ekB, Dictionary<int, PetLevel> EvHSgt7QX8VsEd0yEZ, Dictionary<int, PetSkillElementInfo> R11rtWo1p4Y2vEtvxF, Dictionary<int, PetSkillInfo> m8Jor90v2RFloePXnt, Dictionary<int, PetSkillTemplateInfo> YHCrVLAS0QefBk0Ur5, Dictionary<int, PetTemplateInfo> w2gAkTbu06gsoxFCZU, Dictionary<int, PetFightPropertyInfo> PnKXOFTHDG3PZUPkBD, Dictionary<int, PetStarExpInfo> w84AGjgion3EqNro8x, Dictionary<int, PetExpItemPriceInfo> PetExpItemPrice)
		{
			using (ProduceBussiness produceBussiness = new ProduceBussiness())
			{
				...
				PetExpItemPriceInfo[] PetTempExpItemPrices = produceBussiness.GetAllPetExpItemPrice();
		...
		public static List<UsersPetInfo> CreateAdoptList(int userID, int playerLevel, int vipLv)
		{
			...
			{
				if (!DropInventory.GetPetDrop(613, 1, ref petIdInfos)) continue;
				if (petIdInfos == null) continue;
				int id = random_0.Next(petIdInfos.Count);
		...
		{
			List<int> idpet = new List<int> { 100101, 110101, 120101, 130101 };
			List<UsersPetInfo> infos = new List<UsersPetInfo>();
		...
		Random random = new Random();
			...
			usersPetInfo.BuildProp();
			usersPetInfo.Skill = UpdateSkillPet(1, info.TemplateID, playerLevel);
			usersPetInfo.SkillEquip = ActiveEquipSkill(1, vipLv);
			return usersPetInfo;
		}
		...
		public static UsersPetInfo CreateNewPet(int vipLv)
		{
			...
			return CreatePet(FindPetTemplate(Convert.ToInt32(strArray[index])), -1, -1, 60, vipLv);
		}
		...
		public static PetExpItemPriceInfo FindPetExpItemPrice(int count)
        {
			...
			return m_expItemPrices.ContainsKey(count) ? m_expItemPrices[count] : null;
		}

		public static void purificar(UsersPetInfo petInfo, bool vida, bool ataque, bool defesa, bool sensivel, bool sorte)
        {
			UsersPetInfo newPet = petInfo;
			PetTemplateInfo info = PetMgr.FindPetTemplate(petInfo.TemplateID);
			Random rd = new Random();
			if (!vida)
			{
				newPet.BloodGrow = rd.Next(info.HighBloodGrow * (rd.Next(1, rd.Next(2, 100)) / 100), info.HighBloodGrow);
			}
			if (!ataque)
			{
				newPet.AttackGrow = rd.Next(info.HighAttackGrow * (rd.Next(1, rd.Next(2, 100)) / 100), info.HighAttackGrow);
			}
			if (!defesa)
			{
				newPet.DefenceGrow = rd.Next(info.HighDefenceGrow * (rd.Next(1, rd.Next(2, 100)) / 100), info.HighDefenceGrow);
			}
			if (!sensivel)
			{
				newPet.AgilityGrow = rd.Next(info.HighAgilityGrow * (rd.Next(1, rd.Next(2, 100)) / 100), info.HighAgilityGrow);
			}

			if (!sorte)
			{
				newPet.LuckGrow = rd.Next(info.HighLuckGrow * (rd.Next(1, rd.Next(2, 100)) / 100), info.HighLuckGrow);
			}
		}
