...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1238 : BasePetEffect
    {
        ...
        {
            AE1238 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1238) as AE1238;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1238(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}