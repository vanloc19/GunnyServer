...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1220 : BasePetEffect
    {
        ...
        {
            AE1220 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1220) as AE1220;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            ...
            {
                List<Player> enemys = player.Game.GetAllEnemyPlayers(player);
                foreach (Player enemy in enemys)
                {
                    enemy.Game.SendPetBuff(enemy, ElementInfo, true);
                    enemy.AddPetEffect(
                        new CE1220(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
