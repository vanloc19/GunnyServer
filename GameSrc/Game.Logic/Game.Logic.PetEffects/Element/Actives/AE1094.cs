...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1094 : BasePetEffect
    {
        ...
        {
            AE1094 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1094) as AE1094;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                {
                    ally.AddPetEffect(
                        new CE1094(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
            }
        }
    }
