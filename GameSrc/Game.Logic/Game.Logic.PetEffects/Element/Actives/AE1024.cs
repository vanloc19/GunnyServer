...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1024 : BasePetEffect
    {
        ...
        {
            AE1024 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1024) as AE1024;
            if (effect != null)
            {
        ...
        private void Player_PlayerAnyShellThrow(Player player)
        {
            if (IsTrigger)
            {
                ...
                foreach (Player ally in allies)
                {
                    ...
                    ally.AddPetEffect(new CE1024(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
                player.Game.SendPetBuff(player, ElementInfo, true);
                player.AddPetEffect(new CE1024(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                IsTrigger = false;
            }
        }
