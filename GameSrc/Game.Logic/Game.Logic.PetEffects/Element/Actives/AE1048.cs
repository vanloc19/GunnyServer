...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1048 : BasePetEffect
    {
        ...
        {
            AE1048 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1048) as AE1048;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(new CE1048(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}