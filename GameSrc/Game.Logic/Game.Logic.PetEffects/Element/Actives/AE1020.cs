...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1020 : BasePetEffect
    {
        ...
        {
            AE1020 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1020) as AE1020;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(new CE1020(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}