...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1178 : BasePetEffect
    {
        ...
        {
            AE1178 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1178) as AE1178;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1178(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}