...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1068 : BasePetEffect
    {
        ...
        {
            AE1068 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1068) as AE1068;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                player.AddPetEffect(new CE1068(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}