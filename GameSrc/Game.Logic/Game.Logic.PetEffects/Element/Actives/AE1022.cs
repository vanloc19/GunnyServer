...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1022 : BasePetEffect
    {
        ...
        {
            AE1022 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1022) as AE1022;
            if (effect != null)
            {
        ...
        private void Player_PlayerAnyShellThrow(Player player)
        {
            if (IsTrigger)
            {
                ...
                foreach (Player ally in allies)
                {
                    ...
                    ally.AddPetEffect(
                        new CE1022(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
