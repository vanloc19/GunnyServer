...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1101 : BasePetEffect
    {
        ...
        {
            AE1101 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1101) as AE1101;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                {
                    ally.AddPetEffect(
                        new CE1101(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
            }
        }
    }
