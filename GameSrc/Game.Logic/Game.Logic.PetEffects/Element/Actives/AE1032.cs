...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1032 : BasePetEffect
    {
        ...
        {
            AE1032 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1032) as AE1032;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(new CE1032(1, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}