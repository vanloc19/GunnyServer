...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1093 : BasePetEffect
    {
        ...
        {
            AE1093 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1093) as AE1093;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                foreach (Player ally in allies)
                {
                    ...
                    ally.AddPetEffect(
                        new CE1093(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
            }
        }
    }
