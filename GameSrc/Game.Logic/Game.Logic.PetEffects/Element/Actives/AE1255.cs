...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1255 : BasePetEffect
    {
        ...
        {
            AE1255 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1255) as AE1255;
            if (effect != null)
            {
        ...
        private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
        {
            if (IsTrigger)
            {
                IsTrigger = false;
                target.AddPetEffect(
                    new CE1255(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
