...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1179 : BasePetEffect
    {
        ...
        {
            AE1179 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1179) as AE1179;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1179(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}