...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1271 : BasePetEffect
    {
        ...
        {
            AE1271 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1271) as AE1271;
            if (effect != null)
            {
        ...
        private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
        {
            ...
            {
                target.AddPetEffect(
                    new CE1271(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString(), living), 0);
                IsTrigger = false;
            }
        }
