...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1047 : BasePetEffect
    {
        ...
        {
            AE1047 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1047) as AE1047;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(new CE1047(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}