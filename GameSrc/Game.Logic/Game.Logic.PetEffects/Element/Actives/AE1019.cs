...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1019 : BasePetEffect
    {
        ...
        {
            AE1019 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1019) as AE1019;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(new CE1019(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}