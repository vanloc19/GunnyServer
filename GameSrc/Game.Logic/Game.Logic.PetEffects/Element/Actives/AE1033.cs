...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1033 : BasePetEffect
    {
        ...
        {
            AE1033 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1033) as AE1033;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(new CE1033(1, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}