...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1100 : BasePetEffect
    {
        ...
        {
            AE1100 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1100) as AE1100;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                {
                    ally.AddPetEffect(
                        new CE1100(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
            }
        }
    }
