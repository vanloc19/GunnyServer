...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1212 : BasePetEffect
    {
        ...
        {
            AE1212 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1212) as AE1212;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1212(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
