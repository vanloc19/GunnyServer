...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1198 : BasePetEffect
    {
        ...
        {
            AE1198 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1198) as AE1198;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1198(1, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
