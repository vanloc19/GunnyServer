...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1266 : BasePetEffect
    {
        ...
        {
            AE1266 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1266) as AE1266;
            if (effect != null)
            {
        ...
        private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
        {
            if (IsTrigger)
            {
                ...
                m_added = 0;
                target.AddPetEffect(
                    new CE1266(0, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
