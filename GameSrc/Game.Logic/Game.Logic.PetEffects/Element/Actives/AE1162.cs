...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1162 : BasePetEffect
    {
        ...
        {
            AE1162 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1162) as AE1162;
            if (effect != null)
            {
        ...
        private void Player_PlayerBuffSkillPet(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                foreach (Player enemy in enemies)
                {
                    ...
                    enemy.AddPetEffect(
                        new CE1162(0, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
            }
        }
