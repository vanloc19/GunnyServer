...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1023 : BasePetEffect
    {
        ...
        {
            AE1023 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1023) as AE1023;
            if (effect != null)
            {
        ...
        private void Player_PlayerAnyShellThrow(Player player)
        {
            if (!this.IsTrigger)
                return;
            foreach (Player allTeamPlayer in player.Game.GetAllTeamPlayers((Living)player))
            {
                allTeamPlayer.Game.SendPetBuff((Living)allTeamPlayer, this.Info, true);
                allTeamPlayer.AddPetEffect((AbstractPetEffect)new CE1023(2, this.m_probability, this.m_type, this.m_currentId, this.m_delay, this.Info.ID.ToString()), 0);
            }
            ...
            /*if (IsTrigger)
            {
                ...
                foreach (Player ally in allies)
                {
                    ...
                    ally.AddPetEffect(
                        new CE1023(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
                player.AddPetEffect(new CE1023(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                IsTrigger = false;
            }*/
        }
