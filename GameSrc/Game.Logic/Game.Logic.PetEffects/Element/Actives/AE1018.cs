...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1018 : BasePetEffect
    {
        ...
        {
            AE1018 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1018) as AE1018;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(new CE1018(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}