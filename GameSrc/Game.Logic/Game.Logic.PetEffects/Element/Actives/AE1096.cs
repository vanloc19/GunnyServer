...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1096 : BasePetEffect
    {
        ...
        {
            AE1096 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1096) as AE1096;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                {
                    ally.AddPetEffect(
                        new CE1096(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
            }
        }
    }
