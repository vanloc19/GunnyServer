...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1221 : BasePetEffect
    {
        ...
        {
            AE1221 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1221) as AE1221;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                foreach (Player enemy in enemys)
                {
                    ...
                    enemy.AddPetEffect(
                        new CE1221(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
