...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1183 : BasePetEffect
    {
        ...
        {
            AE1183 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1183) as AE1183;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                foreach (Player ally in allies)
                {
                    ...
                    ally.Game.SendPetBuff(player, ElementInfo, true);
                    ally.AddPetEffect(
                        new CE1183(currentCount, m_probability, m_type, m_currentId, m_delay,
                            ElementInfo.ID.ToString()), 0);
                }
            }
        }
    }
