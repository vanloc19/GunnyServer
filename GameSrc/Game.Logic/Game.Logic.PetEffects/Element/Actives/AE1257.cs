...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1257 : BasePetEffect
    {
        ...
        {
            AE1257 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1257) as AE1257;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                foreach (Player ally in allies)
                {
                    ...
                    ally.AddPetEffect(
                        new CE1257(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
