...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1184 : BasePetEffect
    {
        ...
        {
            AE1184 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1184) as AE1184;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1184(0, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}