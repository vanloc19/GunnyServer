...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1117 : BasePetEffect
    {
        ...
        {
            AE1117 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1117) as AE1117;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                player.AddPetEffect(
                    new CE1117(1, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}