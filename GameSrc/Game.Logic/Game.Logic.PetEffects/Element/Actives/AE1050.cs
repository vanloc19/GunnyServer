...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1050 : BasePetEffect
    {
        ...
        {
            AE1050 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1050) as AE1050;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                player.AddPetEffect(
                    new CE1050(0, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
            }
        }
    }
}