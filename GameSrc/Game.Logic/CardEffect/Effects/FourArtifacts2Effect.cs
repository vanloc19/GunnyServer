...
namespace Game.Logic.CardEffect.Effects
{
    public class FourArtifacts2Effect : BaseCardEffect
    {
        ...
        {
            if (living.CardEffectList.GetOfType(eCardEffectType.FourArtifacts2) is FourArtifacts2Effect effect)
                return true;
            else
        ...
        {
            //player.BeforeTakeDamage += ChangeProperty; //kích hoạt khi bị đối phương bắn trúng
            player.AfterKillingLiving += Player_AfterKillingLiving;//kích hoạt khi bắn trúng đối phương, bắn hụt ko tính.
        }
