...
namespace Game.Logic.CardEffect.Effects
{
    public class ShadowDevil4Effect : BaseCardEffect
    {
        ...
        {
            if (living.CardEffectList.GetOfType(eCardEffectType.ShadowDevil4) is ShadowDevil4Effect effect)
                return true;
            else
        ...
        {
            if (living.Game is PVEGame && (living.Game as PVEGame).Info.ID == 4)
            {
                if (criticalAmount > 0)
